...
public class Blinky extends Ghost {
    ...
    private int[] scatterCoords = {612, -84};

    public Blinky(Game game, Board board, Pacman pacman, float x, float y) {
            ...
            if (eaten) wantsDirection = pathfind(board, currentDirection, ghostHouseCoords[0], ghostHouseCoords[1], false);
                ...
                switch (state) {
                    case 0:
                        ...
                        break;
                    case 1:
                        wantsDirection = pathfind(board, currentDirection, (int)pacman.getX(), (int)pacman.getY(), false);
                        break;
                }
            }
        }
    ...
    public void render(Graphics2D g) {
        ...
        if (freightened) {
            ...
            switch (currentDirection) {
                case 'w':
                    g.drawImage(Assets.blinkyU[animator[animatorCounter]], (int)x-(width/2) + screenXOffset, (int)y-(height/2) + screenYOffset + MAP_Y_OFFSET, width, height, null);
                    break;
                case 'a':
                    g.drawImage(Assets.blinkyL[animator[animatorCounter]], (int)x-(width/2) + screenXOffset, (int)y-(height/2) + screenYOffset + MAP_Y_OFFSET, width, height, null);
                    break;
                case 's':
                    g.drawImage(Assets.blinkyD[animator[animatorCounter]], (int)x-(width/2) + screenXOffset, (int)y-(height/2) + screenYOffset + MAP_Y_OFFSET, width, height, null);
                    break;
                case 'd':
                    g.drawImage(Assets.blinkyR[animator[animatorCounter]], (int)x-(width/2) + screenXOffset, (int)y-(height/2) + screenYOffset + MAP_Y_OFFSET, width, height, null);
                    break;
            }
        }
    }
