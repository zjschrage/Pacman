...
public class Clyde extends Ghost {
    ...
    private int[] scatterCoords = {0, 744};

    public Clyde(Game game, Board board, Pacman pacman, float x, float y) {
            ...
            if (eaten) wantsDirection = pathfind(board, currentDirection, ghostHouseCoords[0], ghostHouseCoords[1], false);
                ...
                switch (state) {
                    case 0:
                        ...
                        break;
                    case 1:
                        if (l2Norm((int)x, (int)y, (int)pacman.getX(), (int)pacman.getY()) >= 8 * board.getTileSize()) wantsDirection = pathfind(board, currentDirection, (int)pacman.getX(), (int)pacman.getY(), false);
                        else wantsDirection = pathfind(board, currentDirection, scatterCoords[0], scatterCoords[1], false);
                        break;
                }
            }
        }
    ...
    public void render(Graphics2D g) {
        ...
        if (freightened) {
            ...
            switch (currentDirection) {
                case 'w':
                    g.drawImage(Assets.clydeU[animator[animatorCounter]], (int)x-(width/2) + screenXOffset, (int)y-(height/2) + screenYOffset + MAP_Y_OFFSET, width, height, null);
                    break;
                case 'a':
                    g.drawImage(Assets.clydeL[animator[animatorCounter]], (int)x-(width/2) + screenXOffset, (int)y-(height/2) + screenYOffset + MAP_Y_OFFSET, width, height, null);
                    break;
                case 's':
                    g.drawImage(Assets.clydeD[animator[animatorCounter]], (int)x-(width/2) + screenXOffset, (int)y-(height/2) + screenYOffset + MAP_Y_OFFSET, width, height, null);
                    break;
                case 'd':
                    g.drawImage(Assets.clydeR[animator[animatorCounter]], (int)x-(width/2) + screenXOffset, (int)y-(height/2) + screenYOffset + MAP_Y_OFFSET, width, height, null);
                    break;
            }
        }
    }
